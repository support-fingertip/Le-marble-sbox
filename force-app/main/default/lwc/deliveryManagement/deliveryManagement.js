...
import searchSalesConfirmations from '@salesforce/apex/DeliveryManagementController.searchSalesConfirmations';
import getAvailableProducts from '@salesforce/apex/DeliveryManagementController.getAvailableProducts';
import createDeliveryGroup from '@salesforce/apex/DeliveryManagementController.createDeliveryGroup';
import getVehicleTypeOptions from '@salesforce/apex/DeliveryManagementController.getVehicleTypeOptions';
import getDriverPicklistOptions from '@salesforce/apex/DeliveryManagementController.getDriverPicklistOptions';
...
export default class DeliveryManagement extends LightningElement {
    ...
    async searchSalesConfirmations() {
        try {
            ...
            this.salesConfirmations = results.map(sc => ({
                label: sc.Name + (sc.Product_Category__c ? ` (${sc.Product_Category__c})` : ''),
                value: sc.Id,
                details: {
                    name: sc.Name,
                    opportunityName: sc.Opportunity__r.Name,
                    company: sc.Opportunity__r.Company__c,
                    customerName: sc.Opportunity__r.Customer__r.Name,
                    phone: sc.Opportunity__r.Phone__c,
                    address: {
                        street: sc.Opportunity__r.Address__Street__s,
                        city: sc.Opportunity__r.Address__City__s,
                        state: sc.Opportunity__r.Address__StateCode__s,
                        postalCode: sc.Opportunity__r.Address__PostalCode__s,
                        country: sc.Opportunity__r.Address__CountryCode__s
                    },
                    stage: sc.Opportunity__r.Stage__c,
                    type: sc.Opportunity__r.Type__c,
                    oppOwner: sc.Opportunity__r.Owner?.Name,
                    totalAmount: sc.Total_Amount__c,
                    ...
                    opportunityId: sc.Opportunity__c,
                    cartLineItems: sc.Cart_Line_Items__r || [],
                    preferredDeliveryDate: sc.Preferred_Delivery_Date__c
                }
            }));
        } catch (error) {
            this.showToast('Error', error.body?.message || 'Error searching sales confirmations', 'error');
        }
    }
    ...
    handleSalesConfirmationSelect(event) {
        ...
        if (selectedSC) {
            ...
            this.selectedOpportunityId = selectedSC.details.opportunityId;
    ...
    async loadInitialData() {
        ...
        try {
            ...
            this.availableProducts = availableProductsResult.map(product => ({
                ...product,
                selected: false,
                deliveryQuantity: product.Quantity__c,
                displayName: `${product.Product__r.Name} (${product.Sales_Confirmation__r.Name}) - Available: ${product.Quantity__c}`
            }));
    ...
    handleProductSelection(event) {
        ...
        if (product) {
            ...
            if (product.selected) {
                product.deliveryQuantity = product.Quantity__c;
            }
            this.validateForm();
        }
    }
    ...
    handleDeliveryQuantityBlur(event) {
        ...
        if (product) {
            ...
            } else if (newQuantity > product.Quantity__c) {
                product.deliveryQuantity = product.Quantity__c;
    ...
    validateForm() {
        ...
        const hasValidQuantities = this.availableProducts.every(product => 
            !product.selected || (product.deliveryQuantity > 0 && product.deliveryQuantity <= product.Quantity__c)
        );
    ...
    async handleCreateDeliveryGroup() {
        try {
            ...
            this.showToast('Success', 'Delivery Group created successfully', 'success');
            ...
            this.showToast('Error', error.body?.message || 'Failed to create Delivery Group', 'error');
        }
    }
    ...
    get selectedSalesConfirmationInfo() {
        ...
        return {
            ...
            opportunityName: this.selectedSalesConfirmation.opportunityName,
