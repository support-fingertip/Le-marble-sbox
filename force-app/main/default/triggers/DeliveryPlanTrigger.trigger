trigger DeliveryPlanTrigger on Delivery_Group__c (
    ...
    if (Trigger.isAfter && Trigger.isInsert) {
        DeliveryPlanTriggerHandler.afterInsert(Trigger.new);
         DeliveryPlanTriggerHandler.onCreate(Trigger.new);
    }

    // Notify Admin when Delivery Plan is marked Delivered
    // Update Order Product quantities
    if (Trigger.isAfter && Trigger.isUpdate) {
        DeliveryPlanTriggerHandler.afterUpdate(Trigger.new,Trigger.oldMap );

        DeliveryPlanQuantityHandler.handleAfterUpdate(Trigger.new,Trigger.oldMap);
		DeliveryPlanTriggerHandler.onStatusChange(Trigger.new, Trigger.oldMap);
    }
